...
/**
 * Accepts both business JWTs and portal JWTs.
 * Both carry businessId so downstream handlers work identically.
 * Sets req.businessId and req.portalUserId (portal only).
 */
@Injectable()
export class CombinedAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      this.logger.warn(`Combined JWT verification failed: ${err.message}`);
